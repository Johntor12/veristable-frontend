...
const SmartContractAddress = () => {
    return (
        <button className='flex flex-row gap-[0.333vw] hover:cursor-pointer active:cursor-pointer border-1 border-[#D5D7DA] rounded-[0.208vw] w-[10.833vw] aspect-[156/36] justify-center items-center'>
            ...
            <p className='text-center text-[#0A0D12] font-jakarta text-[#0A0D12] text-[0.833vw]'>0x12345.....987654</p>
        </button>
    )
}
...
const SmartContractInformation = () => {
    return (
        <button className='flex flex-row gap-[0.333vw] px-[0.667vw] hover:cursor-pointer active:cursor-pointer border-1 border-[#D5D7DA] rounded-[0.208vw] w-[10.833vw] aspect-[156/36] justify-between items-center'>
            <p className='text-center text-[#0A0D12] font-jakarta text-[#0A0D12] text-[0.833vw] leading-[1.25vw] bg'>Gak Tau ini apa</p>
    ...
    return (
        <div className="flex flex-row w-[49.722vw] aspect-[716/100] gap-[1.111vw]">
            ...
            <div className=' flex flex-col text-[2.5vw] text-black font-jakarta w-[41.667vw] aspect-[596/99]'>
                <p className='text-[2.693vw] text-black font-bold font-jakarta'>Fresh Bali Villa</p>
    ...
    return (
        <div className="flex flex-col items-center p-[1.994vw] w-[51.667vw] aspect-[744/259] rounded-[0.694vw] border-2 border-[#E9EAEB] gap-[1.111vw]">
            <div className="flex flex-col w-full font-jakarta gap-[0.833vw]">
                <p className="font-jakarta text-[1.528vw] text-black font-bold leading-[2.292vw]">Contract Checklist</p>
                {/* Progress Bar */}
                ...
                <p className="text-[0.833vw] text-[#905AD4] font-jakarta">{completedCount}/{steps.length} tasks completed</p>
                <div className="flex flex-col justify-start gap-[0.667vw]">
                {steps.map((step) => (
                    <div key={step.id} className={`flex flex-row gap-[0.333vw] items-center ${step.completed ? 'text-[#31006E]' : 'text-[#C5C4C8]'}`}>
                        <div className={`flex w-[1.111vw] aspect-square justify-center items-center rounded-full border-1 border-[#E9EAEB] ${step.completed ? 'bg-transparent' : 'bg-transparent'}`}>
                            {step.completed && <HiCheck className='w-[1.25vw] aspect-square text-[#039855]' />}
                        </div>
                        <p className={`text-[0.972vw] font-jakarta ${step.completed ? 'text-[#039855]' : 'text-black'}`}>{step.label}</p>
                    </div>
                ))
                }
            </div>
