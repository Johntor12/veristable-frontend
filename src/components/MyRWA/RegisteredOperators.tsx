    ...
    return (
        <div className="flex flex-col bg-transparent p-[1.994vw] w-[51.667vw] aspect-[744/319] rounded-[0.694vw] border-2 border-[#D5D7DA] gap-[1.833vw]">
            <div className="flex flex-col gap-[1.111vw]">
                <h2 className="font-jakarta text-[1.25vw] text-black font-bold leading-[2.292vw] border-b-1 border-[#D5D7DA]">Registered Operators/Underwriter</h2>
            ...
            <div className="flex flex-row justify-between items-center w-full bg-transparent border-b-1 border-[#D5D7DA] text-center pb-[0.667vw]">
                <p className="font-jakarta text-[0.833vw] text-black leading-[1.25vw]">OPERATOR</p>
                <p className="font-jakarta text-[0.833vw] text-black leading-[1.25vw]">TOTAL VALUE RESTAKE</p>
                
            </div>
            <div className="flex flex-col gap-[1vw]">
                    {dummyOperators.map((operator) => (
                    <div key={operator.id} className="flex flex-row justify-between items-center w-full border-b border-[#D5D7DA] pb-[0.333vw]">
                        <div className="flex flex-row items-center gap-[0.833vw]">
                            <div className="w-[1.944vw] aspect-square relative rounded-full overflow-hidden">
                                <Image src={operator.logoUrl} alt={operator.name} layout="fill" objectFit="cover" />
                            </div>
                            <span className="font-jakarta text-center font-medium text-[0.972vw] text-black">{operator.name}</span>
                        </div>
                        <span className="font-jakarta  text-center font-medium text-[0.972vw] text-black">{operator.totalValueRestake} ETH</span>
                    </div>
                    ))}
                </div>
            </div>
